import { Project, SyntaxKind, type ImportDeclaration } from "ts-morph";
...
for (const sourceFile of sourceFiles) {
  ...
  const hasImport = sourceFile.getImportDeclaration((decl: any) => decl.getModuleSpecifierValue() === "@/lib/sanitize");
  if (!hasImport) {
  ...
  const targetCalls = callExpressions.filter((c: any) => c.getExpression().getText() === "setResult");
